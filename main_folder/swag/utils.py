...
def eval(loader, model, criterion, cuda=True, regression=False, verbose=False, model_type="autre"):
    loss_sum = 0.0
    correct = 0.0
    ...
    with torch.no_grad():
        ...
        if model_type == "NLP":
            for i, (text, offsets, cls) in enumerate(loader):
                if cuda:
                ...
                loss_sum += loss.item() * text.size(0)

                if not regression:
                    pred = output.data.argmax(1, keepdim=True)
                    correct += pred.eq(cls.data.view_as(pred)).sum().item()
...
def predict(loader, model, verbose=False, type_model='autre'):
    predictions = list()
    targets = list()
...
def bn_update(loader, model, verbose=False, subset=None, type_model='autres', **kwargs):
    """
    ...
    with torch.no_grad():
        ...
        if type_model == "NLP":
            for input, offsets, _ in loader:
                input = input.cuda(non_blocking=True)
                offsets = offsets.cude(non_blocking=True)

                input_var = torch.autograd.Variable(input)
                offsets_var = torch.autograd.Variable(offsets_var)
                ...
                model(input_var, offsets_var, **kwargs)
                n += b
